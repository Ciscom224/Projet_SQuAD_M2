...
1.  [cite_start]**Fine-tuning :** Entraîner au moins 3 modèles (ex: DistilBERT, RoBERTa, CamemBERT) sur SQuAD[cite: 4].
2.  [cite_start]**Analyse Comparative :** Évaluer les modèles selon les métriques F1-Score, Exact Match (EM) et le temps d'inférence[cite: 6].
3.  [cite_start]**Interface Utilisateur :** Développer une application Web (Streamlit/FastAPI) permettant aux utilisateurs de poser des questions sur leurs propres textes ou fichiers.
4.  [cite_start]**Déploiement :** Rendre l'application accessible via Hugging Face Spaces[cite: 7, 14].
